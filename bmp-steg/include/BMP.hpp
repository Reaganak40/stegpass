...
private:
    ...
    /// <summary>
    /// Ensures that a message found is valid, in that is only contains ASCII characters.
    /// </summary>
    /// <param name="message">The message to validate.</param>
    /// <returns> The validated message, if it is in fact valid, otherwise returns an empty string.</returns>
    std::string ValidateMessage(const std::vector<uint8_t>& messageBytes) const;
};