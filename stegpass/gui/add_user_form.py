...
class AddUserForm(tk.Frame):
    def __init__(self, master, **kwargs):
        ...
        self.master_password_entry.insert(0, "Enter Master Password")
    ...
    def clear_master_password_entry(self, event):
        if self.master_password_entry.get() == "Enter Master Password":
            self.master_password_entry.delete(0, tk.END)
            self.master_password_entry.config(fg='black', show='*')
            
    def restore_master_password_entry(self, event):
        if not self.master_password_entry.get():
            self.master_password_entry.insert(0, "Enter Master Password")
            self.master_password_entry.config(fg='grey', show='')
            
    def clear_confirm_master_password_entry(self, event):
    ...
    def restore_confirm_master_password_entry(self, event):
